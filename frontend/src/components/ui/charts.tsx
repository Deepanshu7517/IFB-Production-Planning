// import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, type XAxisTickContentProps, ReferenceLine } from 'recharts';
...
const BarChartWithMultiXAxis = () => {
  const maxValue = Math.max(...data.map(d => Math.max(d.uv, d.pv)));
  return (
    <div className='flex flex-col bg-white border border-gray-300 rounded-md p-2 lg:col-span-8'>
      <p className='text-2xl font-bold text-gray-600'>Production Schedule</p>
      <hr className='text-gray-400' />
      <BarChart
        style={{ width: '100%', maxHeight: '28vh', aspectRatio: 1.618 }}
        responsive
        data={data}
        margin={{
          top: 25,
      ...
      >
        ...
        <Tooltip />
        <Legend wrapperStyle={{ paddingTop: '1em' }} />
        <Bar dataKey="pv" fill="#B9D0B4" />
        <Bar dataKey="uv" fill="#C3D9ED" />
        <ReferenceLine
          y={maxValue}
          stroke="#475569"
          // strokeDasharray="5 5"
